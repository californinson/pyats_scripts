        ...
        for line in output.splitlines():
            ...
            if m:
                ...
                if group['origin_codes']:
                    ...
                    group_num = group['number'].strip()

                    m1 = re.compile(r'^(?P<metric>[0-9]+) +(?P<locprf>[0-9]+) +(?P<weight>[0-9]+)$').match(group_num)

                    m2 = re.compile(r'^(?P<value>[0-9]+)(?P<space>\s{2,20})(?P<weight>[0-9]+)$').match(group_num)

                    m3 = re.compile(r'^(?P<weight>[0-9]+)$').match(group_num)

                    m4 = re.compile(r'^(?P<locprf>(\d+)) +(?P<weight>(\d+))$').match(group_num)
